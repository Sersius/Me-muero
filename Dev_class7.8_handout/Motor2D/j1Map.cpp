...
void j1Map::Path(int x, int y)
{
	...
	while (curr != visited.start->data && visited.find(curr) != -1) 
	{
		path.PushBack(curr);
...
void j1Map::PropagateDijkstra()
{
	...
	if (frontier.Pop(curr))
	{
		...
		for (uint i = 0; i < 4; ++i)
		{
			...
			if ((cost_so_far[neighbors[i].x][neighbors[i].y] == 0 || new_cost < cost_so_far[neighbors[i].x][neighbors[i].y]) && (neighbors[i].x < data.width && neighbors[i].x >= 0 && neighbors[i].y < data.height && neighbors[i].y >= 0) && neighbors[i] != iPoint(7,13))
			{
